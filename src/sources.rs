use crate::parse::{bell_plus, periods_io};
use crate::School;
...
pub async fn get_school(
    ...
    if is_stale(&path, cache_stale_after).await {
        ...
        match fetch.await {
            Ok(fetched) => write_to_cache(&path, &fetched).await?,
            Err(err) => {
                eprintln!("{}", err);
            }
        }
...
async fn fetch_school(id: &SchoolId) -> Result<School, reqwest::Error> {
    let source = reqwest::get(format!("https://bell.plus/api/data/{}", id))
        .await?
        .json::<bell_plus::BellSource>()
        .await?;

    let school = bell_plus::to_school(source);
